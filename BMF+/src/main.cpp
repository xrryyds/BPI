...
void testAppendRandW(){
    std::string File = "test.txt";
    int x1 = 11111;
    int x2 = 22222;
    int x3 = 33333;

    std::fstream ofs(File, std::ios::in | std::ios::out | std::ios::binary);
    if (ofs.is_open())
    {
        ofs.seekp(static_cast<std::streamoff>(sizeof(unsigned int)) * 5, std::ios::beg);
        ofs.write(reinterpret_cast<char *>(&x1), sizeof(unsigned int));
        ofs.write(reinterpret_cast<char *>(&x2), sizeof(unsigned int));
        ofs.write(reinterpret_cast<char *>(&x3), sizeof(unsigned int));
        ofs.close();
    }

    std::vector<unsigned int> result;
    readMask(File, result);
