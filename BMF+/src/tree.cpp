...
void BMFTree::updataIndex(int featureID, std::vector<unsigned int> leafMaskBuffer, std::vector<unsigned int> middleMaskBuffer, int theNumberOfAllRoot, int middleID, int leafID)
{
    ...
    std::ofstream ofs;
    ofs.open("../file/firstIndexFile/Index_" + std::to_string(featureID), std::ios::out | std::ios::app);
    if (ofs.is_open())
    {
...
unsigned int BMFTree::Hybrid_query(std::vector<unsigned int> maskList)
{
...
void BMFTree::query(std::vector<int> featureList, std::vector<std::string> &resultData)
{
    ...
    for (int i = 0; i < theNumberOfRoot; i++)
    {
        ...
        {
            maskBuffer.push_back(BMFRootNode[featureList[f]][i]);
        }
        tmp0 = findAllOn(Hybrid_query(maskBuffer));
        for (int j = 0; j < tmp0.size(); j++)
        {
            ...
            tmp1 = findAllOn(Hybrid_query(maskBuffer));
            for (int k = 0; k < tmp1.size(); k++)
            {
                ...
                tmp2 = findAllOn(Hybrid_query(maskBuffer));
                for (int l = 0; l < tmp2.size(); l++)
                {
                    result.push_back(i * 32 * 32 * 32 + tmp0[j] * 32 * 32 + tmp1[k] * 32 + tmp2[l]);
                }
                tmp2.clear();
            }
            indexLeaf += tmp1.size();
...
void BMFTree::updataTree(int theNumberOfAllData)
{
    ...
    int k;
    std::ofstream ofs;
    int allRoot = std::ceil(static_cast<float>(std::ceil(static_cast<float>(std::ceil(static_cast<float>(theNumberOfAllData) / static_cast<float>(32))) / static_cast<float>(32))) / static_cast<float>(32));
